...
- `dist/usb.guardian-0.1.0_beta3-x86_64-1.txz`
...
Download the PLG and TXZ from the [v0.1.0-beta3 release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v0.1.0-beta3). Without network access, place the TXZ in the plugin's private directory and the PLG in the root `/boot/config/plugins` directory:

```bash
mkdir -p /boot/config/plugins/usb.guardian
cp usb.guardian-0.1.0_beta3-x86_64-1.txz /boot/config/plugins/usb.guardian/
...
5. 重新扫描全部进程的 fd/cwd/root/map、挂载命名空间、loop、swap 和 block holder，并确认同一个 `shfs` PID、`fuse.shfs` 挂载和 `/mnt/user` 目录 I/O 正常。
...
从 [v0.1.0-beta3 Release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v0.1.0-beta3) 下载 PLG 和 TXZ。没有网络连接时，把 TXZ 放进插件私有目录，把 PLG 放到 `/boot/config/plugins` 根目录：
