...
2. Click the static eject icon next to the mounted device name. USB Guardian performs no eligibility request merely to display this icon.
...
- `dist/usb.guardian-0.1.0_beta5-x86_64-1.txz`
...
Download the PLG and TXZ from the [v0.1.0-beta5 release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v0.1.0-beta5). Without network access, place the TXZ in the plugin's private directory and the PLG in the root `/boot/config/plugins` directory:

```bash
mkdir -p /boot/config/plugins/usb.guardian
cp usb.guardian-0.1.0_beta5-x86_64-1.txz /boot/config/plugins/usb.guardian/
...
1. 在 UD 中正常挂载 U 盘。
2. 在已挂载设备名称旁点击静态弹出图标。仅显示该图标不会触发任何资格检查请求。
...
从 [v0.1.0-beta5 Release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v0.1.0-beta5) 下载 PLG 和 TXZ。没有网络连接时，把 TXZ 放进插件私有目录，把 PLG 放到 `/boot/config/plugins` 根目录：
