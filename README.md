...
- `dist/usb.guardian-2026.07.19b-x86_64-1.txz`
...
Download the PLG and TXZ from the [v2026.07.19b release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v2026.07.19b). Without network access, place the TXZ in the plugin's private directory and the PLG in the root `/boot/config/plugins` directory:

```bash
mkdir -p /boot/config/plugins/usb.guardian
cp usb.guardian-2026.07.19b-x86_64-1.txz /boot/config/plugins/usb.guardian/
...
从 [v2026.07.19b Release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v2026.07.19b) 下载 PLG 和 TXZ。没有网络连接时，把 TXZ 放进插件私有目录，把 PLG 放到 `/boot/config/plugins` 根目录：
