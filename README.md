...
- `dist/usb.guardian-0.1.0_beta4-x86_64-1.txz`
...
Download the PLG and TXZ from the [v0.1.0-beta4 release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v0.1.0-beta4). Without network access, place the TXZ in the plugin's private directory and the PLG in the root `/boot/config/plugins` directory:

```bash
mkdir -p /boot/config/plugins/usb.guardian
cp usb.guardian-0.1.0_beta4-x86_64-1.txz /boot/config/plugins/usb.guardian/
...
1. 在 UD 中正常挂载 U 盘。
2. 在设备名称旁点击弹出图标。
3. 若显示黄色警告图标，点击它查看阻止原因、占用进程和处理建议。
...
从 [v0.1.0-beta4 Release](https://github.com/xO-ox-ai/unraid-usb-guardian/releases/tag/v0.1.0-beta4) 下载 PLG 和 TXZ。没有网络连接时，把 TXZ 放进插件私有目录，把 PLG 放到 `/boot/config/plugins` 根目录：
